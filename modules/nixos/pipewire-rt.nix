{
  ...
  jsonGenerate = pkgs.formats.json {};
  qr = "${toString cfg.quantum}/${toString cfg.rate}";

  customConfig = jsonGenerate.generate "99-custom.conf" {
    "context.properties" = {
      "link.max-buffers" = 16;
      "log.level" = 2;
  ...
  customPulseConfig = jsonGenerate.generate "99-pulse-custom.conf" {
    "context.properties" = {
      "log.level" = 2;
    };
    "context.modules" = [
...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    environment.etc."pipewire/pipewire.conf.d/99-custom.conf".source = customConfig;
    environment.etc."pipewire/pipewire.conf.d/99-pulse-custom.conf".source = customPulseConfig;
  };
}
