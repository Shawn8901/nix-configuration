...
in {
  options = {
    shawn8901 = {
      result = mkOption { type = types.unspecified; };
          ...
          options = {
            ...
            homeManager = mkOption {
              ...
              type = types.attrsOf (types.submodule
                ({ name, config, ... }: { options = baseConfigType; }));
            };
          } // baseConfigType;
        }));
      };
  ...
  config.shawn8901.system-generator.generateSystem = mapAttrs (name: conf:
    withSystem conf.hostPlatform.system ({ system, inputs', self', ... }:
      let
                    ...
                    imports = [
                      ({ config, ... }: {
                        sops = {
                          ...
                          defaultSymlinkPath =
                            "/run/user/${toString user.uid}/secrets";
                          defaultSecretsMountPoint =
                            "/run/user/${toString user.uid}/secrets.d";
                        };
                        xdg.configFile."nix/inputs/nixpkgs".source =
                          conf.nixpkgs.outPath;
