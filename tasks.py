...
ALL_HOSTS = DeployGroup([DeployHost("pointalpha"), DeployHost(
    "pointjig"), DeployHost("tank"),  DeployHost("shelter"), ])
...
def sync_config(host: DeployHost):
    ...
    host.run_local(
        f"rsync {' --exclude '.join([''] + RSYNC_EXCLUDES)} -vaF --delete -e ssh . {host.user}@{host.host}:/etc/nixos")
    return flake_path
...
def run_command(host: DeployHost, command: NixosRebuildCommand, log: str):
    ...
    print(log)
    host.run(
        f"nixos-rebuild {command.value} --build-host localhost --target-host {target_host} --flake $(realpath {flake_path})")
...
def deploy(_, hosts="localhost"):
    ...
    def run(h: DeployHost) -> None:
        run_command(h, NixosRebuildCommand.SWITCH,
...
def build(_, hosts="localhost"):
    g: DeployGroup = parse_host_arg(hosts)
    g.run_function(lambda h: run_command(
        h, NixosRebuildCommand.BUILD, "🏗️ Building system."))
...
def test(_, hosts="localhost"):
    g: DeployGroup = parse_host_arg(hosts)
    g.run_function(lambda h: run_command(
        h, NixosRebuildCommand.TEST, "🤞 Testing configuration."))
